...
import getQuietMessage from '../data/randomThoughts';
import { useState}from 'react';
import {getRandomCaveMessage, getContextualCaveMessage } from '../data/caveMessages';

//import { getFullInteractiveStatement, createEnhancedRoomDescription } from '../utils/descriptionUtils';
  


// Function to get valid adjacent position for wumpus to move to
const getAdjacentWumpusPosition = (currentPosition, positions, playerPosition, specialRooms) => {
  ...
  // Possible movement directions: +1, -1, +2, -2
  ...
  // First try adjacent positions (+1 or -1)
...
playSandCreatureShriekSound
  ...
  // Reference to track same room entry attempts
  ...
  const [showNixieDisplay, setShowNixieDisplay] = useState(false);
    // Add this line right before all your function definitions
  // This will make isFloating available to all functions in the hook
...
// Function to add visual warning effect to a pit room button 
// This function uses a different approach than other highlight functions to make it more subtle
// Modified version that uses the connected rooms information


    // The animation property breakdown:
    // animation: [name] [duration] [timing-function] [iteration-count]
    // Current: 1s (duration) ease-in-out (timing) 3 (repeats 3 times)
    // 
    // To make it slower/more noticeable, change to:
    // animation: pit-warning-${uniqueID} 2s ease-in-out 5;
    // This would make each flash 2 seconds long and repeat 5 times
...
const addPitWarningEffect = (pitRoom) => {
  ...
  // Function to calculate threat perceptions based on current position
  ...
  // Get a random empty position (for wizard teleport)
...
// Function to update room description and room-specific effects
const updateRoomDescription = (position) => {
  // Add this helper function at the top
 // Updated checkAndDisplayExit function for useGameLogic.js
const checkAndDisplayExit = (roomInfo, position) => {
  ...
  if (roomInfo?.isExitRoom || roomInfo?.special === 'exit' || position === positions.exitPosition) {
    ...
    if (hasWyrmglass) {
      ...
      // If lantern is ON, ladder is INVISIBLE
      if (hasActiveLantern) {
      ...
      // Check if ladder has been extended
...
const roomInfo = roomDescriptionMap[position];  
  ...
  if (hasTeleportInSpecialRooms) {
    ...
    if (roomInfo) {
      ...
      // Only add hint if it's not already there AND it's not the pool room
  ...
  if (hasHiddenDoorInSpecialRooms) {
    ...
    if (roomInfo) {
      ...
      // Only add hint if it's not already there
 ...
 // Function to refresh room with lantern
const refreshRoomWithLantern = () => {
  ...
  if (roomInfo.poolDisturbed) {
      ...
      // Lantern is active - enhanced description
      if (roomInfo.enhancedText) {
        ...
        // Store common interactive item patterns
const commonPatterns = [
  { regex: / On a small ledge, you spot a <span class=['"]interactive-item['"][^>]*>flask of oil<\/span>\./i, itemType: 'torch_oil' },
  { regex: / In the corner, you spot a <span class=['"]interactive-item['"][^>]*>tattered cloth<\/span>\./i, itemType: 'invisibility_cloak' },
  { regex: / Among them, you spot unusual <span class=['"]interactive-item['"][^>]*>salt-like crystals<\/span> with a subtle blue glow\./i, itemType: 'cave_salt' },
  { regex: / Yellow <span class=['"]interactive-item['"][^>]*>sulfur crystals<\/span> line the walls\./i, itemType: 'sulfur_crystal' }
];
        const foundPatterns = [];
      ...
      //  const itemRegex = /<span class=['\"]interactive-item['\"][^>]*>.*?<\/span>/g;
        ...
        if (specialHints.length) newDescription += " " + specialHints.join(" ");
        if (foundPatterns.length) {
          foundPatterns.forEach(pattern => {
            const itemTypeRegex = new RegExp(`data-item=['"]${pattern.itemType}['"]`, 'i');
            if (!itemTypeRegex.test(newDescription)) newDescription += " " + pattern.fullText;
          });
        }
        ...
        setRoomDescriptionMap(prev => ({
          ...prev,
          [currentPosition]: { ...prev[currentPosition], displayText: newDescription }
        }));
...
// This is a utility function to use whenever creating enhanced descriptions
// Add this to your useGameLogic.js file
...
const createEnhancedRoomDescription = (originalText, enhancedText) => {
  ...
  // Helper function to get the full statement containing an interactive item
...
// Helper function to extract item type from span
...
const revealHiddenItems = (position, inventory) => {
  ...
  // Check if item has already been revealed
...
// Function to update all perceptions and room info
// Function to update all perceptions and room info
...
// Helper function to generate perception based on special type
...
console.log('Wumpus:', positions.wumpusPosition);
  // Update goblin cooldown on movement
  ...
  // Reset water sprite toll status
...
if ((position === positions.pitPosition1 || position === positions.pitPosition2) && !isFloating()) {
  ...
  // Check if you encountered the wumpus
...
if (position === positions.exitPosition) {
  ...
  if (!hasWyrmglass) {
    ...
    // Don't show any exit messages
    return;
  }
  
  // Check if player has an active lantern
  // Skip if lantern was just toggled
  ...
  // Only show exit message if lantern is NOT active
  if (!hasActiveLantern) {
...
// Hide treasure display after 5 seconds
setTimeout(() => {
...
shouldReturn = true;
  ...
  // Base message for special rooms
  if (position > 30) {
    roomMessage = "You are in a hidden chamber.";
  }

  // Check for special room items
...
setTimeout(() => {
  ...
  if (roomNumber === giftShopRoom) {
    ...
    // Check if goblin is on cooldown
    if (goblinCooldown > 0) {
    ...
    // Check if player has gold coins
  ...
  const initializeLooseRocks = (startingRoom) => {
   ...
   // const updatedDesc = {...(roomDescriptionMap[startingRoom] || {})};
    ...
    // IMPORTANT: Store the original description
    updatedDesc.originalText = updatedDesc.text;
  ...
  const startGame = (testRoom = null) => {
    // Get a safe starting room
    let startingRoom;
    console.log("CHECKING FOR LANTERN ROOM");
  
    // Find the room with the lantern
    for (let i = 1; i <= 30; i++) {
    ...
    if (testRoom !== null) {
      // Use the test room if provided
      startingRoom = parseInt(testRoom);
   // } else if (startingRoomFixed !== null) {
      // Use the fixed room (which we set to the lantern room)
   //   startingRoom = startingRoomFixed;
   //   console.log(`Starting in fixed room: ${startingRoom} (lantern room)`);
    } else {
      // Get all occupied dangerous rooms
      const dangerousRooms = [
        positions.wumpusPosition,
        positions.pitPosition1,
        positions.pitPosition2,
        positions.batPosition,
        positions.exitPosition,  // ADD THIS LINE - CRITICAL!
      treasureMap
      ];
 ...
 Object.entries(roomDescriptionMap).forEach(([roomId, roomDesc]) => {
   ...
   // ADD THIS: Check for gift shop room
      if (roomDesc.text && (
...
// Also add the designated gift shop position if it exists
   ...
   // NEW: Also exclude rooms adjacent to the exit
    ...
    for (let i = 1; i <= 30; i++) {
      if (!dangerousRooms.includes(i) && 
          ...
          i !== positions.exitPosition) { // ADD THIS EXTRA CHECK
        safeRooms.push(i);
      }
    }
    ...
    if (safeRooms.length > 0) {
      ...
      for (let i = 1; i <= 30; i++) {
        if (!dangerousRooms.includes(i) && 
            ...
            i !== positions.exitPosition) {  // ADD THIS CHECK
          allSafeRooms.push(i);
        }
      }
    ...
    // Find any room that isn't the exit
    for (let i = 1; i <= 30; i++) {
  ...
  console.log(`Distance from exit: ${roomConnections[startingRoom]?.includes(positions.exitPosition) ? "1 (adjacent)" : "2+ rooms away"}`);
    ...
    setHistory([startingRoom]);
      ...
      // Call with explicit debugging

 // Initialize special rooms and oil flask locations
...
//addItemToInventory('lantern'); // TESTING LINE - REMOVE BEFORE PRODUCTION

//addItemToInventory('wyrmglass') 
//addItemToInventory('wizard_journal') 
//addItemToInventory('crystal_orb') 

 //initializeCleanWaterSource();
  ...
  setFungiWarning(false);
    ...
    // Give player a starting oil flask
// Check if player already has oil
...
if (!hasOil) {
 ...
 //addItemToInventory('torch_oil');
  //addItemToInventory('invisibility_cloak')
  //addItemToInventory('crystal_orb')
  addItemToInventory('loose_rocks')
  addItemToInventory('rusty_key')
  //addItemToInventory('crystal_orb')
  
  //addGiftShopItemToInventory('canvas_bag');
  //addItemToInventory('lantern')
 // addGiftShopItemToInventory('wyrmglass');
addItemToInventory('gold_coins');
//addItemToInventory('sulfur_crystal');
 addItemToInventory("lantern");
  // Append message about having oil

// Add these lines to instantly collect all treasures:
setCollectedTreasures(['ruby', 'medallion', 'statue', 'amulet']);
setHasMap(true);

// Update the canvas bag to show it's filled
setInventory(prev => prev.map(item => {
  if ((item.originalId || item.id) === 'canvas_bag') {
    return {
      ...
      description: 'A sturdy canvas bag containing 4 treasures.',
      treasureCount: 4
    };
  }
  return item;
...
const handleGuess = (event) => {
 ...
 // Check if we're in shop mode FIRST
  if (shopMode) {
    ...
    // Clear Wumpus attack timer if it exists
    if (wumpusAttackTimer) {
      clearTimeout(wumpusAttackTimer);
      setWumpusAttackTimer(null);
    }
...
if (currentPosition && specialRooms[currentPosition]?.hasSandCreature && !isFloating()) {
  ...
  if (previousRoom !== guess && specialRooms[currentPosition]?.sandCreatureActive) {
    // Check if player has protective items - ADD SULFUR CRYSTAL CHECK
    ...
    if (hasProtectiveOrb || hasAmulet || hasSulfurCrystal) {
      ...
      // NEW: Determine which protection is being used
          ...
          return item;
            ...
            window.dispatchEvent(new CustomEvent('orb_protection_activated', {
              detail: { 
                ...
                duration: 6000 // 6 seconds to match your existing glow duration
              }
            }));
  
            // sand wraith is not permanently deactivated
            setSpecialRooms(prev => ({
              ...prev,
              [currentPosition]: {
      ...
      // No change to the "no protection" outcome
  ...
  // No change to the "going back" outcome
...
console.log("showNixieDisplay", showNixieDisplay);
    ...
    // Add this line to check for golden compass
...
//duplicate fungi code was here. 


  // Continue with normal room navigation if no sand creature attack
    ...
    // If there's a current position, check if the guess is a valid connection
   /* if (currentPosition && roomConnections[currentPosition]) {
      // Only allow movement to connected rooms
      if (!roomConnections[currentPosition].includes(guess)) {
        setMessage(`There's no tunnel from room ${currentPosition} to room ${guess}. Try one of the connected rooms.`);
        return;
      }
    }  */
   ...
   /* if (currentPosition && roomConnections[currentPosition]) {
    ...
    if (history.length >= 2 && 
      ...
      if (wizardSummonCountRef.current >= 3 && !window.WIZARD_FREED) { 
        // After going back and forth 3 times, summon the wizard
  ...
  if (specialRooms[guess]?.hasItem) {
    ...
    // EXCLUDE SPELLBOOK from auto-collection
    if (itemId !== 'spellbook') {
      ...
      //This chamber is filled with unusual crystals that seem to glow faintly with their own inner light.
    ...
    if (guess === positions.batPosition) {
      ...
      console.log('Player entered room with bat:', guess);
        // NEW CODE: Check for wumpus t-shirt
      ...
      // After showing the bat message for 3 seconds, move the player and update
      setTimeout(() => {
        ...
        setHistory([...history, guess, randomRoom]); // Record both positions
        ...
        do {
          ...
          newBatPosition === positions.pitPosition2 ||
          newBatPosition === treasureMap// Not where pit 2 is
        );
...
if (floatingMovesLeft === 1 && (guess === positions.pitPosition1 || guess === positions.pitPosition2)) {
  ...
  setTimeout(() => {
    ...
    // Check for other encounters
    checkPosition(guess, foundItem);
    
 // Check if player is wearing wumpus t-shirt
const wearingTshirt = inventory.some(item => 
  (item.originalId || item.id) === 'wumpus_tshirt' && item.equipped
);

// Increase aggression chance if wearing the t-shirt
...
if (Math.random() < wumpusAggressionChance) {
      ...
      setInventory(prev => prev.map(item => {
        if ((item.originalId || item.id) === 'sulfur_crystal') {
          ...
          if (newMovesRemaining <= 0) {
            ...
            // Remove it from inventory (after 10 moves)
            setTimeout(() => {
              setInventory(current => current.filter(i => 
                (i.originalId || i.id) !== 'sulfur_crystal'
              ));
  ...
  if (guess === shiftingRoomId) {
    ...
    setRoomDescriptionMap(prev => ({
      ...
      [shiftingRoomId]: {
        ...
        lastUpdatedIndex: 0,
        isStabilized: true,
        // Force the original text
        text: shiftingRoomDescriptions[0]?.text || prev[shiftingRoomId]?.originalDescription || originalRoomDescription
      }
    }));
  }
}
  ...
  // CRITICAL: Check if player has stabilizer
  const hasStabilizer = inventory.some(item => 
    (item.originalId || item.id) === 'reality_stabilizer'
  );
  ...
  // CRITICAL: Check if room has been stabilized previously
  ...
  if (currentPosition !== SHIFTING_ROOM_TO_UPDATE && hasVisitedShiftingRoom) {
    ...
    try {
      ...
      // Clean interactive items from the description
  ...
  // Expand the input handler to support text commands
const handleChange = (event) => {
  ...
  // Treasure hunt state (these ARE passed to useGameLogic)
  setTreasureMap(null);
  setTreasurePieces([]);
  setCollectedTreasures([]);
  setHasMap(false);
  setShowNixieDisplay(false)
  // setMapClue is NOT passed, so skip it
  
  // Treasure display state (if these are passed)
  ...
  // Wizard-related (only what's passed)
  ...
  // IMPORTANT: Return statement at the end of the function to pass to other files functions
  return {
    handleGuess,
    handleChange,
    resetGame,
